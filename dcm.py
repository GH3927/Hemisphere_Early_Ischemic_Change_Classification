...
def bsb_windowing(img, window_center=None, window_width=None, rescale=True):
...
def beo_windowing(img, window_center=None, window_width=None, rescale=True):
    brain_img = windowing(img, 40, 80, rescale)
    eic_img = windowing(img, 40, 40, rescale)
    oi_img = windowing(img, 20, 40, rescale)
    
    beo_img = np.zeros((brain_img.shape[0], brain_img.shape[1], 3))
    beo_img[:, :, 0] = brain_img
    beo_img[:, :, 1] = eic_img
    beo_img[:, :, 2] = oi_img
...
def gradient_bsb_windowing(img, window_center=None, window_width=None, rescale=True):
    brain_img = windowing(img, 40, 80, rescale)
...
def gradient_beo_windowing(img, window_center=None, window_width=None, rescale=True):
    brain_img = windowing(img, 40, 80, rescale)
    eic_img = windowing(img, 40, 40)
    oi_img = windowing(img, 20, 40)
    combo = (brain_img*0.3 + eic_img*0.5 + oi_img*0.2)
...
def sigmoid_bsb_windowing(img, window_center=None, window_width=None, rescale=True):
...
def sigmoid_beo_windowing(img, window_center=None, window_width=None, rescale=True):
    brain_img = sigmoid_windowing(img, 40, 80, rescale)
    eic_img = sigmoid_windowing(img, 40, 40, rescale)
    oi_img = sigmoid_windowing(img, 20, 40, rescale)
...
def sigmoid_gradient_bsb_windowing(img, window_center=None, window_width=None, rescale=True):
    brain_img = sigmoid_windowing(img, 40, 80, rescale)
    subdural_img = sigmoid_windowing(img, 80, 200, rescale)
    bone_img = sigmoid_windowing(img, 600, 2000, rescale)
...
def sigmoid_gradient_beo_windowing(img, window_center=None, window_width=None, rescale=True):
    brain_img = sigmoid_windowing(img, 40, 80, rescale)
    eic_img = sigmoid_windowing(img, 40, 40, rescale)
    oi_img = sigmoid_windowing(img, 20, 40, rescale)
    combo = (brain_img*0.35 + eic_img*0.5 + oi_img*0.15)
    combo_norm = (combo - np.min(combo)) / (np.max(combo) - np.min(combo))    
